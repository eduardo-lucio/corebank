import { pool } from "../../config/db";
import { FastifyReply, FastifyRequest } from "fastify";
import '@fastify/jwt';

export async function getAccountBalance(req: FastifyRequest, res: FastifyReply) {
    try {
        const bal = await pool.query(
            'SELECT balance FROM accounts WHERE user_id = $1',
            [req.user.sub]
        );

        if (bal.rows.length === 0) {
            return res.status(404).send({
                statusCode: 404,
                error: 'Not Found',
                message: 'Conta bancária não encontrada'
            });
        }

        return res.status(200).send({
            statusCode: 200,
            balance: Number(bal.rows[0].balance)
        });
    } catch (error) {
        req.log.error(error);
        return res.status(500).send({
            statusCode: 500,
            error: 'Internal Server Error',
            message: 'Erro interno ao consultar saldo'
        });
    }
}

export async function getAccountStatement(req: FastifyRequest, res: FastifyReply) {
    try {
        const userAccount = await pool.query(
            'SELECT * FROM accounts WHERE user_id = $1;',
            [req.user.sub]
        );

        if (userAccount.rows.length === 0) {
            return res.status(404).send({
                statusCode: 404,
                error: 'Not Found',
                message: 'Conta não encontrada'
            });
        }

        const myAccount = userAccount.rows[0];
        const transactions = await pool.query(
            'SELECT id, amount, type, status, created_at FROM transactions WHERE sender_account_id = $1 OR receiver_account_id = $1 ORDER BY created_at DESC LIMIT 50',
            [myAccount.id]
        );

        return res.status(200).send({
            account_number: myAccount.account_number,
            balance: Number(myAccount.balance),
            total_transactions: transactions.rows.length,
            transactions: transactions.rows
        });
    } catch (err) {
        req.log.error(err);
        return res.status(500).send({
            statusCode: 500,
            error: 'Internal Server Error',
            message: 'Erro interno ao consultar extrato'
        });
    }
}